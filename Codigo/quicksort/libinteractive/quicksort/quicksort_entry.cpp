	...
	while (readfull(&__in, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x13857295u: {
				...
				uint32_t __cookie;
				int HacerParticion_inicio;
				readfull(&__in, &HacerParticion_inicio, sizeof(int), 1);
				int HacerParticion_fin;
				readfull(&__in, &HacerParticion_fin, sizeof(int), 1);
				readfull(&__in, &__cookie, sizeof(__cookie), 1);

				int result =
				HacerParticion(HacerParticion_inicio, HacerParticion_fin);
...
int main(int argc, char* argv[]) {
	...
	openstream(&__in, "./quicksort_in", O_RDONLY);
	openstream(&__out, "./quicksort_out", O_WRONLY);
