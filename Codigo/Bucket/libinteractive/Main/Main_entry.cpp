...
static struct __stream __Bucket_in, __Bucket_out;

void __NOINLINE __libinteractive_init() {
	openstream(&__Bucket_in, "./Bucket_in", O_WRONLY);
	openstream(&__Bucket_out, "./Bucket_out", O_RDONLY);
}
...
static void __message_loop_Bucket(int __current_function, int __noreturn) {
	uint32_t __msgid;
	while (readfull(&__Bucket_out, &__msgid, sizeof(__msgid), 0)) {
...
void inicio(int N, int M[]) {
	...
	writefull(&__Bucket_in, &__msgid, sizeof(__msgid));
	writefull(&__Bucket_in, &N, sizeof(int));
	writefull(&__Bucket_in, M, sizeof(int) * (N));
	writefull(&__Bucket_in, &__cookie, sizeof(__cookie));
	writeflush(&__Bucket_in);

	__message_loop_Bucket(__msgid,  0 );
	uint32_t __cookie_result = 0;
	readfull(&__Bucket_out, &__cookie_result, sizeof(__cookie_result), 1);
...
int pregunta(int x) {
	...
	writefull(&__Bucket_in, &__msgid, sizeof(__msgid));
	writefull(&__Bucket_in, &x, sizeof(int));
	writefull(&__Bucket_in, &__cookie, sizeof(__cookie));
	writeflush(&__Bucket_in);

	__message_loop_Bucket(__msgid,  0 );
	int __ans = 0;
	readfull(&__Bucket_out, &__ans, sizeof(__ans), 1);
	uint32_t __cookie_result = 0;
	readfull(&__Bucket_out, &__cookie_result, sizeof(__cookie_result), 1);
