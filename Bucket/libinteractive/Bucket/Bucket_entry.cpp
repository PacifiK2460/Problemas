	...
	while (readfull(&__in, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x13857295u: {
				...
				int (*inicio_M) = (int(*))malloc(sizeof(int) * inicio_N);
				readfull(&__in, inicio_M, sizeof(int) * inicio_N, 1);
				readfull(&__in, &__cookie, sizeof(__cookie), 1);

				inicio(inicio_N, inicio_M);
			...
			case 0x71651df7u: {
				// Main -> Bucket.pregunta
				uint32_t __cookie;
				int pregunta_x;
				readfull(&__in, &pregunta_x, sizeof(int), 1);
				readfull(&__in, &__cookie, sizeof(__cookie), 1);
...
int main(int argc, char* argv[]) {
	...
	openstream(&__in, "./Bucket_in", O_RDONLY);
	openstream(&__out, "./Bucket_out", O_WRONLY);
