...
static struct __stream __ordenador_in, __ordenador_out;

void __NOINLINE __libinteractive_init() {
	openstream(&__ordenador_in, "./ordenador_in", O_WRONLY);
	openstream(&__ordenador_out, "./ordenador_out", O_RDONLY);
}
...
static void __message_loop_ordenador(int __current_function, int __noreturn) {
	uint32_t __msgid;
	while (readfull(&__ordenador_out, &__msgid, sizeof(__msgid), 0)) {
		if (__msgid == __current_function) return;
		switch (__msgid) {
			case 0x71651df7u: {
				// ordenador -> Main.guardar
				uint32_t __cookie;
				int guardar_indice;
				readfull(&__ordenador_out, &guardar_indice, sizeof(int), 1);
				readfull(&__ordenador_out, &__cookie, sizeof(__cookie), 1);

				int result =
				guardar(guardar_indice);

				writefull(&__ordenador_in, &__msgid, sizeof(__msgid));
				writefull(&__ordenador_in, &result, sizeof(result));
				writefull(&__ordenador_in, &__cookie, sizeof(__cookie));
				writeflush(&__ordenador_in);
				break;
			}

			case 0x685d93e7u: {
				// ordenador -> Main.obtener
				uint32_t __cookie;
				int obtener_indice;
				readfull(&__ordenador_out, &obtener_indice, sizeof(int), 1);
				readfull(&__ordenador_out, &__cookie, sizeof(__cookie), 1);

				int result =
				obtener(obtener_indice);
			...
			case 0x43f73b41u: {
				// ordenador -> Main.desplazar
				uint32_t __cookie;
				int desplazar_indice;
				readfull(&__ordenador_out, &desplazar_indice, sizeof(int), 1);
				readfull(&__ordenador_out, &__cookie, sizeof(__cookie), 1);

				bool result =
				desplazar(desplazar_indice);
			...
			case 0x3a61de2fu: {
				// ordenador -> Main.insertar
				uint32_t __cookie;
				int insertar_indice;
				readfull(&__ordenador_out, &insertar_indice, sizeof(int), 1);
				readfull(&__ordenador_out, &__cookie, sizeof(__cookie), 1);

				insertar(insertar_indice);
...
void ordenar(int n) {
	...
	const uint32_t __cookie = 0x1b602b0bu;

	writefull(&__ordenador_in, &__msgid, sizeof(__msgid));
	writefull(&__ordenador_in, &n, sizeof(int));
	writefull(&__ordenador_in, &__cookie, sizeof(__cookie));
	writeflush(&__ordenador_in);

	__message_loop_ordenador(__msgid,  0 );
	uint32_t __cookie_result = 0;
	readfull(&__ordenador_out, &__cookie_result, sizeof(__cookie_result), 1);
