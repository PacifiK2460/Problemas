...
static void __message_loop_Main(int __current_function, int __noreturn) {
	...
	while (readfull(&__in, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x13857295u: {
				// Main -> ordenador.ordenar
				uint32_t __cookie;
				int ordenar_n;
				readfull(&__in, &ordenar_n, sizeof(int), 1);
...
int guardar(int indice) {
	...
	writefull(&__out, &__msgid, sizeof(__msgid));
	writefull(&__out, &indice, sizeof(int));
	writefull(&__out, &__cookie, sizeof(__cookie));
	writeflush(&__out);
...
int obtener(int indice) {
	...
	const uint32_t __cookie = 0x6331532u;
...
bool desplazar(int indice) {
	const uint32_t __msgid = 0x43f73b41u;
	const uint32_t __cookie = 0x7699a72cu;
...
void insertar(int indice) {
	const uint32_t __msgid = 0x3a61de2fu;
	const uint32_t __cookie = 0x63fdbb7bu;
...
int main(int argc, char* argv[]) {
	...
	openstream(&__in, "./ordenador_in", O_RDONLY);
	openstream(&__out, "./ordenador_out", O_WRONLY);
